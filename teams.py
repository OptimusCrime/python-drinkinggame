...
def main():
    ...
    if len(names_lines) > 0:
        ...
        for i in range(0, len(names_lines)):
            ...
            print display_name + repeat_to_length(' ', 119 - len(display_name.decode('utf-8'))) + '║'
...
def generate_team():
    ...
    while len(names_lines) > 0:
        ...
        if i % 2 != 0:
            ...
            print '║ ' + bcolors.OKGREEN + 'Lag ' + str(team_number) + ': ' + bcolors.ENDC + other + ' og ' + names_lines[rand].strip() + repeat_to_length(' ', 119 - len(team_tempstring.decode('utf-8'))) + '║'
...
def append_names():
    ...
    while True:
        # Getting response
        name = str(raw_input(bcolors.OKGREEN + '> Navn: ' + bcolors.ENDC))
        print ''
